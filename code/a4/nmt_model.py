...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        tmpX = self.post_embed_cnn(torch.permute(X, (1, 2, 0))).permute(2, 0, 1)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(pack_padded_sequence(tmpX, source_lengths))
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)
        last_hidden_concat = torch.concat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(last_hidden_concat)
        last_cell_concat = torch.concat((last_cell[0], last_cell[1]), dim=1)
